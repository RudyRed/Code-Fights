...
// there is a root to leaf path in t such that the sum of vertex values equals s.
//
// Example
//
// For
...
//   this.right = null;
// }
const hasPathWithGivenSum = function (t, s) {
  if (!t) return s === 0;
  s -= t.value;
  if (!t.right && !t.left) return s === 0;

  if (t.left && hasPathWithGivenSum(t.left, s)) return true;
  if (t.right && hasPathWithGivenSum(t.right, s)) return true;

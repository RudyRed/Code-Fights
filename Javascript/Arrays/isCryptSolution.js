...
//
// It is guaranteed that solution only contains entries for the letters present in
// crypt and that different letters have different values.
//
// [output] boolean
//
// Return true if the solution represents the correct solution to the cryptarithm
// crypt, otherwise return false.

function isCryptSolution(crypt, solution) {
  let solObj = {};
  for (let tuple of solution) {
    solObj[tuple[0]] = tuple[1];
  }
  let numberObj = {0: '', 1: '', 2: ''};
  for (let i = 0; i < crypt.length; i++) {
    let word = crypt[i];
    for (let j = 0; j < word.length; j++) {
      if (j === 1 && numberObj[i] == 0) return false;
      numberObj[i] += solObj[word[j]];
    }
  }
  return Number(numberObj[0]) + Number(numberObj[1]) == numberObj[2];
};
